...
const connect = (localPort, cb) => {
  ...
  const REMOTE_HOST = process.env.WORMHOLE_REMOTE_HOST
  const REMOTE_PORT = process.env.WORMHOLE_REMOTE_PORT
  ...
  if (!REMOTE_HOST)
    return cb(new Error('The environment variable WORMHOLE_REMOTE_HOST must be specified'))

  if (!REMOTE_PORT || !utils.isInt(REMOTE_PORT))
    return cb(new Error('The environment variable WORMHOLE_REMOTE_PORT must be a valid port'))
